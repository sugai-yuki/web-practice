    ...
    <div v-for="(value, key, index) in shares" :key="index">
      <div class="message">
        <div class="flex">
          ...
          <img class="icon" src="../assets/heart.png" @click="fav(key)" />
  ...
  methods: {
      ...
      if(result) {
        this.shares[index].like.forEach((element) => {
          if(element.user_id == this.$store.state.user.id) {
            axios({
              method: "delete",
              url: "https://cryptic-hollows-00296.herokuapp.com/api/like",
              data: {
        ...
        axios
          .post("https://cryptic-hollows-00296.herokuapp.com/api/like", {
            share_id: this.shares[index].item.id,
            user_id: this.$store.state.user.id,
          })
          .then((response) => {
    ...
    async getShares(){
      ...
      let shares = await axios.get(
        "https://cryptic-hollows-00296.herokuapp.com/api/shares"
      );
      for(let i = 0; i < shares.data.data.length; i++){
        await axios
          .get(
            "https://cryptic-hollows-00296.herokuapp.com/api/shares/" + shares.data.data[i].id
          )
          .then((response) => {
